...
import type { ApiResponse, ListResponse, ChannelGroup } from '@/types'
...
export async function listLoadBalanceStrategies() {
  const response = await client.get<ApiResponse<LoadBalanceStrategy[]>>('/channel-group/load-balance-strategies')
  return response.data.data
}
